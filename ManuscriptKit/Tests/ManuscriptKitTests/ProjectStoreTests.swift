...
@Suite struct ProjectStoreTests {
    ...
    @Test func addScenePathsAreNumberedSluggedAndUnique() throws {
        try withTemporaryDirectory { url in
            ...
            #expect(manuscript.chapters.count == 1)
            ...
            trimmed.chapters[0].scenes.removeLast(2)
            ...
            var withChapter = try store.manifest()
            withChapter.chapters.append(Chapter(title: "Netherfield"))
            try store.saveManifest(withChapter, message: "Add chapter")
            let inSecond = try store.addScene(title: "The Ball", toChapter: withChapter.chapters[1].id, text: "Dancing.\n")
